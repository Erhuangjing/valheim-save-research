...
tools/     解析与改写脚本
tools/xibpbuilder_reference/   服务端实时构建插件的参考代码骨架（重建；实机编译已通过）
```
...
| `bp_reconcile.py` | **落地离线对账**（期望件 vs 存档实况，hash+位置双匹配 y 量化；坑 C.2 入库版；验收前置「索引恒等式」检查 + `precise_scan` 精确扫描；selftest 四案全过） |
...
本仓库公开前对文档与脚本做统一脱敏（历史执行情况见 issue #6 的清点）：

| 类别 | 处理 |
|---|---|
| 用户名 / Windows 账户 / Steam ID | 替换为占位符 |
| 世界名 / 服务器名 / 角色名 | 统一写 `WORLD` / `<服务器>` / `<角色>` |
| 服务器密码、世界种子 | 删除或占位 |
| 本机绝对路径（盘符/目录结构） | 替换为 `<存档根>` / `<服务器安装目录>` 等占位符 |
| 项目代号 | 替换为中性词 |

**例外：标识符不算内容**（issue #9 的教训：把插件 GUID 一起替换，导致 cfg 完全不生效）。
GUID、AssemblyName、配置键名、协议字段名、类名这类**参与程序行为的标识符**不做替换；
确需改名（如本仓库对参考插件 GUID 的处理）必须在文档显式声明「已变更、旧配置不通用」。
